...
class NeuralNetWithMask(torch.nn.Module):
    r"""
    Simple example model that also accepts a mask.
    """

    def __init__(self, input_size: int, embedding_size: int, hidden_size: int, num_classes: int):
        super(NeuralNetWithMask, self).__init__()
...
class GradientGraphBuilderTest(unittest.TestCase):
    ...
    def test_save_multi_param_model(self):
        # We need a custom loss function to load the graph in an InferenceSession in ONNX Runtime Web.
        # You can still make the gradient graph with torch.nn.CrossEntropyLoss() and this test will pass.
        loss_fn = binary_cross_entropy_loss
        input_size = 10
        hidden_size = 5
        model = NeuralNetWithMask(input_size=input_size, embedding_size=20, hidden_size=hidden_size, num_classes=2)
        directory_path = Path(os.path.dirname(__file__)).resolve()
        ...
        batch_size = 1
        example_input = (
            torch.randn(batch_size, input_size, requires_grad=True),
            torch.randn(batch_size, hidden_size, requires_grad=True),
        )
        example_labels = torch.tensor([1])
